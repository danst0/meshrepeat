"""Translator-Modul: Skip-Heuristiken + OpenAI-Chat-Antwort-Parsing.

Wir mocken die HTTP-Schicht via httpx.MockTransport, damit weder Netzwerk
noch ein laufender LLM-Server (llama-swap/llama.cpp) nötig sind."""
...
def _chat_content(content: str) -> dict:
    """OpenAI-Chat-Completion-Hülle um einen rohen ``content``-String."""
    return {"choices": [{"message": {"role": "assistant", "content": content}}]}


def _chat_response(lang: str, translated: str) -> dict:
    """OpenAI-Chat-Completion mit JSON-stringified ``{lang, translated}``."""
    return _chat_content(json.dumps({"lang": lang, "translated": translated}))
...
async def test_translate_returns_translation(monkeypatch: pytest.MonkeyPatch) -> None:
    def handler(req: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json=_chat_response("nl", "Mahlzeit"),
        )
...
async def test_translate_already_target_language(monkeypatch: pytest.MonkeyPatch) -> None:
    def handler(req: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json=_chat_response("de", ""))
...
async def test_translate_invalid_inner_json(monkeypatch: pytest.MonkeyPatch) -> None:
    def handler(req: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json=_chat_content("not valid json"))
...
async def test_translate_dropped_when_lang_equals_target(
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json=_chat_response("DE", "Guten Morgen!"))
...
def test_module_logger_present() -> None:
    ...
    # _parse_chat_response() bei Fehlern verwendet.
...
async def test_first_try_succeeds_does_not_retry(
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        calls.append(req)
        return httpx.Response(200, json=_chat_response("nl", "Wie ist der Chef?"))
...
async def test_retry_kicks_in_when_model_echoes_source(
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        ...
        if len(calls) == 1:
            ...
            # zuschlägt).
            return httpx.Response(200, json=_chat_response("nl", "Wie  is de baas?"))
...
async def test_translate_strips_mentions_before_llm(
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(200, json=_chat_response("en", "Guten Abend aus Ost-NL"))
...
async def test_translate_only_mentions_returns_none(
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        calls.append(req)
        return httpx.Response(200, json=_chat_response("en", "..."))
...
async def test_translate_does_not_duplicate_mention(
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json=_chat_response("en", "@[CJWinty] Nachmittag"))
...
async def test_translate_restores_multiple_mentions_in_order(
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json=_chat_response("en", "Hallo zusammen"))
